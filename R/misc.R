...
error_decoding <- function(decoding, states){
    ...
    res$n_injected <- sum((location_inj[-1]-location_inj[-length(location_inj)])!=1)
    location_decoding <- which(decoding)
    res$n_detected <- sum((location_decoding[-1]-location_decoding[-length(location_decoding)])!=1)

    location_TP <- which( states & decoding)
    res$TP <- sum((location_TP[-1]-location_TP[-length(location_TP)])!=1)
    location_FP <- which(!states & decoding)
    res$FP <- sum((location_FP[-1]-location_FP[-length(location_FP)])!=1)
