...
generated quantities {
    ...
    {
        vector[3] alpha[N-1];
        vector[3] logbeta[N-1];
        vector[3] p_FFBS_unorm[N-1];
        vector[3] beta[N-1];
        

        // forward, no longer log
        for(tt in 1:(N-1)){
            alpha[tt] = softmax(gamma[tt]);
        }// end of forward
        // backward algo
        {
            real accub_quiet[2];// used for quiet
            real accub_firing[2];// used for firing, just to avoid -Inf firing can come from anywhere
            real accub_decay[3]; // used for decay
            for (j in 1:3){
                logbeta[N-1, j] = 1;
            }
            for(tforward in 0:(N-1-2)){
                ...
                // from quiet: 
                    // to quiet
                accub_quiet[1] = logbeta[tt,1] + log(theta_quiet[1]) + 
                                  normal_lpdf(yd[tt+1]|0, sqrt(sigma2_noise));
                    // to firing
                accub_quiet[2] = logbeta[tt,2] + log(theta_quiet[2]) + 
                                  exp_mod_normal_lpdf(yd[tt+1] | yd[tt], sqrt(sigma2_noise) , rate_firing );
                
                logbeta[tt-1, 1] = log_sum_exp(accub_quiet);

                // from firing
                    // to firing
                accub_firing[1] = logbeta[tt,2] + log(theta_firing[1]) + 
                                    exp_mod_normal_lpdf(yd[tt+1] | yd[tt], sqrt(sigma2_noise) , rate_firing );
                    // to decay:
                accub_firing[2] = logbeta[tt,3] + log(theta_firing[2]) + 
                                    normal_lpdf(yd[tt+1] | rate_decay * yd[tt] , sqrt(sigma2_noise));
                
                logbeta[tt-1, 2] = log_sum_exp(accub_firing);
                // from decau
                    // to quiet
                accub_decay[1] = logbeta[tt,1] + log(theta_decay[1]) + 
                                    normal_lpdf(yd[tt+1]|0, sqrt(sigma2_noise));
                    // to firing
                accub_decay[2] = logbeta[tt,2] + log(theta_decay[2]) + 
                                    exp_mod_normal_lpdf(yd[tt+1] | yd[tt], sqrt(sigma2_noise) , rate_firing );
                    // to decay
                accub_decay[3] = logbeta[tt,3] + log(theta_decay[3]) + 
                                    normal_lpdf(yd[tt+1] | rate_decay * yd[tt] , sqrt(sigma2_noise));

                logbeta[tt-1,3] = log_sum_exp(accub_decay);
            }// backward loops

            for(tt in 1:(N-1)){
                    beta[tt] = softmax(logbeta[tt]);
            }
        }// end of backward
        ...
        {
            for(tt in 1:(N-1)){
                p_FFBS_unorm[tt] = alpha[tt] .* beta[tt];
                p_FFBS[tt] = normalize(p_FFBS_unorm[tt]);
                FFBS[tt] = categorical_rng(p_FFBS[tt]);
            }
        }
    }
}
