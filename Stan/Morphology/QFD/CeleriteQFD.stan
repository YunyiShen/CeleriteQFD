...
data {
    ...
    real mu0_incre_firing; // prior on how much increase
    real<lower=0> sigma_incre_firing; // prior on how much increase
...
parameters {
    ...
    real logincrem_firing; // must be increasing on average
...
model{
    ...
    sigma2_decay ~ inv_gamma(gamma_decay[1], gamma_decay[2]);
        // mean parameters
    mu_quiet ~ normal(mu0_quiet, sqrt(sigma2_quiet/lambda_quiet));// serve as overall mean 
    ...
    logincrem_firing ~ normal(mu0_incre_firing, sigma_incre_firing);
