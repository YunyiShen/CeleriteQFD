...
// Quiet-Firing-Decay with exponential modified Normal Increase and Decay 
data {
    int<lower=1> N;
...
parameters {
    ...
    real lograte_decay; // the exponent of decaying, must be between 0 and 1 (to decrease)
}
...
model{
    ...
    lograte_firing ~ normal(mu0_rate_firing-0.5*log(sigma2_noise), sigma_rate_firing);
    ...
    for(tt in 2:(N-1)){
        ...
        accu_decay[1] = gamma[tt-1, 2] + log(theta_firing[2]) + 
                  ...
                  //double_exponential_lpdf(yd[tt+1]|0, 1/rate_decay);
        //    came from decay state
...
generated quantities {
    ...
    {   
        ...
        // decay
        best_logp[1,3] = exp_mod_normal_lpdf(yd[2] | 0, sqrt(sigma2_noise) , rate_decay );
