...
template <typename timestep, typename lightcurve,typename termparaS0, typename termparaw0, typename termparaQ, typename termparaeps, typename jitter>
typename boost::math::tools::promote_args<timestep, lightcurve, termparaS0, termparaw0, typename boost::math::tools::promote_args<termparaQ, termparaeps, jitter>::type>::type
logLikSHO(const Eigen::Matrix<timestep, Eigen::Dynamic, 1> & t,
          const Eigen::Matrix<lightcurve, Eigen::Dynamic, 1> & y, 
          const termparaS0 & S0,
          const termparaw0 & w0,
          const termparaQ & Q,
          const termparaeps & eps,  
          const <jitter, Eigen::Dynamic, 1> & diag,
          std::ostream* pstream__){
  typename boost::math::tools::promote_args<timestep, lightcurve, termparaS0, termparaw0, typename boost::math::tools::promote_args<termparaQ, termparaeps, jitter>::type>::type local_scalar_t__;
  typedef local_scalar_t__ fun_return_scalar_t__;
  // declear a SHO term
  SHOTerm<local_scalar_t__> curr_SHOTerm(S0, w0, Q, eps);
  Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, Eigen::Dynamic> c, a, U, V;
  std::tie (c, a, U, V) = curr_SHOTerm.get_celerite_matrices(t,diag);// get those magic matrixes
  Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, Eigen::Dynamic> Z, d_out, S_out, F_out;
  Eigen::Index placeholder;
  placeholder = factor(t, c, a, U, V, a, V, S_out); // to reuse memory
  ...
  solve_lower(t, c, U, W, y, Z, F_out);
  Eigen::Array temp;
  temp = a.array();
  temp = temp.inverse() * Z.array();
  Z = temp.matrix();
  solve_upper(t,c,U,W,y,Z,F_out);
  return(-0.5 * sum(log(a))-0.5 * y.transpose() * Z);
}

template <typename timestep, typename lightcurve,typename termparasigma, typename termparaperiod, typename termparaQ0, typename termparadQ, typename termparaf,typename termparaeps, typename jitter>
typename boost::math::tools::promote_args<timestep, lightcurve, termparasigma, termparaperiod, typename boost::math::tools::promote_args<termparaQ0, termparadQ, termparaf,termparaeps, jitter>::type>::type
logLikRotation(const Eigen::Matrix<timestep, Eigen::Dynamic, 1> & t,
          const Eigen::Matrix<lightcurve, Eigen::Dynamic, 1> & y, 
          const termparasigma & sigma,
          const termparaperiod & period,
          const termparaQ0 & Q0,
          const termparadQ & dQ,
          const termparaf & f,
          const termparaeps & eps, 
          const Eigen::Matrix<jitter, Eigen::Dynamic, 1> & diag,
          std::ostream* pstream__){
  typename boost::math::tools::promote_args<timestep, lightcurve, termparasigma, termparaperiod, typename boost::math::tools::promote_args<termparaQ0, termparadQ, termparaf,termparaeps, jitter>::type>::type local_scalar_t__ ;
  ...
  amp =  sigma * sigma/ (1 + f);
  Q1 = 0.5 + Q0 + dQ;
  w1 = 4 * 3.1415926 * Q1 / (period * std::sqrt(4 * Q1 * Q1 - 1));
  S1 = amp / (w1 * Q1);
  Q2 = 0.5 + Q0;
  w2 = 8 * 3.1415926 * Q2 / (period * std::sqrt(4 * Q2 * Q2 - 1));
  S2 = f * amp / (w2 * Q2);

  // get the two terms and add them up
  SHOTerm<local_scalar_t__> curr_SHOTerm1(S1, w1, Q1, eps);
  SHOTerm<local_scalar_t__> curr_SHOTerm2(S2, w2, Q2, eps);
  Term<local_scalar_t__> Rotation = curr_SHOTerm1 + curr_SHOTerm2;
